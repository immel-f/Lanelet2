...
TEST_F(MLConverterTest, MapData) {  // NOLINT
  ...
  // Verify the crossing is associated with the crosswalk lanelet
  EXPECT_EQ(mapData->associatedCpdLineStringsOfType(2018, LineStringType::ZebraCrossing).size(), 1);
  CompoundLaneLineStringInstancePtr assoCrossing =
...
TEST_F(MLConverterTest, MapDataTrafficElements) {  // NOLINT
  ...
  // // Plot zebra crossings with thick black and white stripes pattern
  // std::vector<Eigen::MatrixXd> zebraCrossings = tfData.compoundLineStringsOfType(LineStringType::ZebraCrossing);
  // for (const auto& mat : zebraCrossings) {
  //   std::vector<double> x;
  //   std::vector<double> y;
  //   x.resize(mat.rows());
  //   y.resize(mat.rows());
  //   Eigen::VectorXd::Map(&x[0], mat.rows()) = mat.col(0);
  //   Eigen::VectorXd::Map(&y[0], mat.rows()) = mat.col(1);
  //   matplot::plot(x, y, "-")->line_width(5).color({0.3, 0.3, 0.3});  // dark gray for zebra
  // }
